...
describe("e2e_token_contract", () => {
  ...
  beforeAll(async () => {
    ...
    asset = await TokenContract.deploy(wallets[0], accounts[0].address)
      .send()
      .deployed();
  ...
  describe("Access controlled functions", () => {
    ...
    it("Add minter as admin", async () => {
      const tx = asset
        ...
        .methods.set_minter(accounts[1].address, true)
        .send();
      const receipt = await tx.wait();
      expect(receipt.status).toBe(TxStatus.MINED);
      expect(await asset.methods.is_minter(accounts[1].address).view()).toBe(
        true
      );
    });
    ...
    it("Revoke minter as admin", async () => {
      const tx = asset
        .withWallet(wallets[1])
        .methods.set_minter(accounts[1].address, false)
        .send();
      const receipt = await tx.wait();
      expect(receipt.status).toBe(TxStatus.MINED);
      expect(await asset.methods.is_minter(accounts[1].address).view()).toBe(
    ...
    describe("failure cases", () => {
      ...
      it("Revoke minter not as admin", async () => {
        await expect(
          asset.methods.set_minter(accounts[0].address, false).simulate()
        ).rejects.toThrowError("Assertion failed: caller is not admin");
      });
    });
  });
    ...
    describe("Public", () => {
      it("as minter", async () => {
        ...
        const tx = asset.methods
          .mint_public(accounts[0].address, amount)
          .send();
        const receipt = await tx.wait();
        expect(receipt.status).toBe(TxStatus.MINED);
        ...
        expect(
          await asset.methods.balance_of_public(accounts[0].address).view()
        ).toEqual(tokenSim.balanceOfPublic(accounts[0].address));
        expect(await asset.methods.total_supply().view()).toEqual(
      ...
      describe("failure cases", () => {
        ...
        it("mint >u120 tokens to overflow", async () => {
          ...
          await expect(
            asset.methods.mint_public(accounts[0].address, amount).simulate()
        ...
        it("mint <u120 but recipient balance >u120", async () => {
          const amount =
            2n ** 120n - tokenSim.balanceOfPublic(accounts[0].address);
          await expect(
            asset.methods.mint_public(accounts[0].address, amount).simulate()
        ...
        it("mint <u120 but such that total supply >u120", async () => {
          const amount =
            2n ** 120n - tokenSim.balanceOfPublic(accounts[0].address);
          await expect(
            asset.methods.mint_public(accounts[1].address, amount).simulate()
          ).rejects.toThrowError("Assertion failed: Overflow");
        });
      });
    });
    ...
    describe("Private", () => {
      ...
      describe("Mint flow", () => {
        ...
        it("redeem as recipient", async () => {
          ...
          expect(await asset.methods.num_shields().view()).toEqual(
            1n
          );
        });
      });
      ...
      describe("failure cases", () => {
        it("try to redeem as recipient (double-spend) [REVERTS]", async () => {
          await expect(
            addPendingShieldNoteToPXE(0, amount, secretHash, txHash)
          ).rejects.toThrowError("The note has been destroyed.");
          await expect(
            asset.methods
              .redeem_shield(accounts[0].address, amount, secret)
              .simulate()
        ...
        it("mint >u120 tokens to overflow", async () => {
          const amount = 2n ** 120n; // SafeU120::max() + 1;
          await expect(
            asset.methods.mint_private(amount, secretHash).simulate()
          ).rejects.toThrowError(
            "Assertion failed: Value too large for SafeU120"
          );
        });
        ...
        it("mint <u120 but recipient balance >u120", async () => {
          const amount =
            2n ** 120n - tokenSim.balanceOfPrivate(accounts[0].address);
  ...
  describe("Transfer", () => {
    describe("public", () => {
      it("transfer less than balance", async () => {
        const balance0 = await asset.methods
          .balance_of_public(accounts[0].address)
          .view();
      ...
      it("transfer to self", async () => {
        ...
        expect(amount).toBeGreaterThan(0n);
        const tx = asset.methods
          .transfer_public(accounts[0].address, accounts[0].address, amount, 0)
          .send();
        const receipt = await tx.wait();
        expect(receipt.status).toBe(TxStatus.MINED);
      ...
      describe("failure cases", () => {
        ...
        it("transfer more than balance on behalf of other", async () => {
          ...
          const balance1 = await asset.methods
            .balance_of_public(accounts[1].address)
            .view();
          const amount = balance0 + 1n;
          const nonce = Fr.random();
          expect(amount).toBeGreaterThan(0n);
          ...
          expect(
            await asset.methods.balance_of_public(accounts[1].address).view()
          ).toEqual(balance1);
        });
    ...
    describe("private", () => {
      it("transfer less than balance", async () => {
        const balance0 = await asset.methods
          .balance_of_private(accounts[0].address)
          .view();
        const amount = balance0 / 2n;
        expect(amount).toBeGreaterThan(0n);
        const tx = asset.methods
          .transfer(accounts[0].address, accounts[1].address, amount, 0)
          .send();
        const receipt = await tx.wait();
        expect(receipt.status).toBe(TxStatus.MINED);
        tokenSim.transferPrivate(
          accounts[0].address,
          accounts[1].address,
          amount
        );
      ...
      it("transfer to self", async () => {
        ...
        expect(amount).toBeGreaterThan(0n);
        const tx = asset.methods
          .transfer(accounts[0].address, accounts[0].address, amount, 0)
          .send();
        const receipt = await tx.wait();
        expect(receipt.status).toBe(TxStatus.MINED);
        tokenSim.transferPrivate(
      ...
      describe("failure cases", () => {
        ...
        it("transfer on behalf of self with non-zero nonce", async () => {
          ...
          await expect(
            asset.methods
              .transfer(accounts[0].address, accounts[1].address, amount, 1)
              .simulate()
          ).rejects.toThrowError("Assertion failed: invalid nonce");
        });
        ...
        it("transfer more than balance on behalf of other", async () => {
          const balance0 = await asset.methods
            ...
            .view();
          const balance1 = await asset.methods
            .balance_of_private(accounts[1].address)
            .view();
          const amount = balance0 + 1n;
          const nonce = Fr.random();
          expect(amount).toBeGreaterThan(0n);
          ...
          expect(
            await asset.methods.balance_of_private(accounts[0].address).view()
          ).toEqual(balance0);
          expect(
            await asset.methods.balance_of_private(accounts[1].address).view()
          ).toEqual(balance1);
        });
  ...
  describe("Shielding (shield + redeem_shield)", () => {
    ...
    it("on behalf of self", async () => {
      ...
      const tx = asset.methods
        .shield(accounts[0].address, amount, secretHash, 0)
        .send();
      const receipt = await tx.wait();
      expect(receipt.status).toBe(TxStatus.MINED);
    ...
    describe("failure cases", () => {
      ...
      it("on behalf of self (invalid nonce)", async () => {
        ...
        await expect(
          asset.methods
            .shield(accounts[0].address, amount, secretHash, 1)
            .simulate()
        ).rejects.toThrowError("Assertion failed: invalid nonce");
      });
  ...
  describe("Unshielding", () => {
    it("on behalf of self", async () => {
      ...
      const tx = asset.methods
        .unshield(accounts[0].address, accounts[0].address, amount, 0)
        .send();
      const receipt = await tx.wait();
      expect(receipt.status).toBe(TxStatus.MINED);
    ...
    describe("failure cases", () => {
      ...
      it("on behalf of self (invalid nonce)", async () => {
        ...
        await expect(
          asset.methods
            .unshield(accounts[0].address, accounts[0].address, amount, 1)
            .simulate()
        ).rejects.toThrowError("Assertion failed: invalid nonce");
      });
  ...
  describe("Burn", () => {
    describe("public", () => {
      it("burn less than balance", async () => {
        ...
        expect(amount).toBeGreaterThan(0n);
        const tx = asset.methods
          .burn_public(accounts[0].address, amount, 0)
          .send();
        const receipt = await tx.wait();
        expect(receipt.status).toBe(TxStatus.MINED);
      ...
      it("burn on behalf of other", async () => {
        ...
        const action = asset
          .withWallet(wallets[1])
          .methods.burn_public(accounts[0].address, amount, nonce);
        const messageHash = computeAuthWitMessageHash(
          accounts[1].address,
          action.request()
        );
      ...
      describe("failure cases", () => {
        it("burn more than balance", async () => {
          ...
          await expect(
            asset.methods
              .burn_public(accounts[0].address, amount, nonce)
              .simulate()
          ).rejects.toThrowError("Assertion failed: Underflow");
        });
    ...
    describe("private", () => {
      ...
      it("burn on behalf of other", async () => {
        ...
        const action = asset
          .withWallet(wallets[1])
          .methods.burn(accounts[0].address, amount, nonce);
        const messageHash = computeAuthWitMessageHash(
          accounts[1].address,
          action.request()
        );
      ...
      describe("failure cases", () => {
        it("burn more than balance", async () => {
          ...
          expect(amount).toBeGreaterThan(0n);
          await expect(
            asset.methods.burn(accounts[0].address, amount, 0).simulate()
          ).rejects.toThrowError("Assertion failed: Balance too low");
        });
        ...
        it("burn on behalf of self with non-zero nonce", async () => {
          ...
          expect(amount).toBeGreaterThan(0n);
          await expect(
            asset.methods.burn(accounts[0].address, amount, 1).simulate()
          ).rejects.toThrowError("Assertion failed: invalid nonce");
        });
  ...
  describe("Attestations", () => {
    ...
    it("Request attestation", async () => {
        const note = await asset.methods
          .last_unattested_note(accounts[0].address, attestor.address)
          .view();

        const txClaim = asset.methods
          .request_attestation(note._value, attestor.address)
          .send();
        const receiptClaim = await txClaim.wait();
        expect(receiptClaim.status).toBe(TxStatus.MINED);
